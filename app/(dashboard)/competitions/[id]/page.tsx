...
export default function CompetitionDetailPage() {
  ...
  const handleStartCompetition = async () => {
    if (!confirm('Start this competition?')) return
  ...
  if (loading) {
    return (
      <div className="page-container-narrow">
        ...
        <div className="skeleton h-40 rounded-xl mb-4"></div>
        <div className="grid grid-cols-3 gap-3 mb-4">
          {[1, 2, 3].map(i => (
            <div key={i} className="skeleton h-20 rounded-xl"></div>
          ))}
        </div>
        <div className="skeleton h-36 rounded-xl"></div>
      </div>
    )
  }
      ...
      <div className="page-container-narrow">
        <div className="card p-8">
          <div className="empty-state">
            ...
            <p className="empty-state-text mb-6">This competition doesn't exist or has been deleted</p>
  ...
  const getHeaderStyle = () => {
    if (competition.status === 'active') return 'bg-emerald-600'
    if (competition.status === 'finished') return 'bg-slate-700'
    return 'bg-white border border-slate-200'
  }

  const getHeaderTextStyle = () => {
    if (competition.status === 'draft') return 'text-slate-900'
    return 'text-white'
  }
  ...
  return (
    <div className="page-container-narrow">
      ...
      {/* Header Card */}
      <div className={`rounded-xl p-6 mb-4 ${getHeaderStyle()}`}>
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="flex-1 min-w-0">
            <StatusBadge />
            <h1 className={`text-xl font-semibold mt-3 ${getHeaderTextStyle()}`}>
              {competition.name}
            </h1>
            {competition.species && (
              <p className={`text-sm mt-1 ${competition.status !== 'draft' ? 'text-white/70' : 'text-slate-500'}`}>
                {competition.species}
              </p>
            )}
          </div>
        ...
        <div className={`flex flex-wrap gap-x-5 gap-y-2 text-sm ${
          competition.status !== 'draft' ? 'text-white/80' : 'text-slate-600'
        }`}>
      ...
      {/* Prize banner */}
      {competition.prize && (
        <div className="prize-banner mb-4">
      ...
      {competition.status === 'active' && (
        <div className="grid grid-cols-3 gap-3 mb-4">
          ...
          >
            <div className="action-btn-icon bg-emerald-500/10 text-emerald-600">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 4v16m8-8H4" />
              </svg>
            </div>
            <span className="action-btn-label">Log catch</span>
          </Link>
          <Link
            href={`/competitions/${competitionId}/leaderboard`}
            className="action-btn"
          >
            <div className="action-btn-icon bg-amber-500/10 text-amber-600">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
              </svg>
            </div>
            <span className="action-btn-label">Standings</span>
          </Link>
          <Link
            href={`/competitions/${competitionId}/captures`}
            className="action-btn"
          >
            <div className="action-btn-icon bg-teal-500/10 text-teal-700">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
            </div>
            <span className="action-btn-label">Gallery</span>
          </Link>
        </div>
      )}
      ...
      {competition.status === 'finished' && (
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-base font-semibold text-slate-900">Final Results</p>
              <p className="text-sm text-slate-500">View complete standings</p>
            </div>
          </div>
              ...
              {acceptedParticipants.map((participant: any, index: number) => (
                ...
                >
                  {participant.profiles?.avatar_url ? (
                    ...
                    <div className="w-9 h-9 rounded-lg bg-teal-700 flex items-center justify-center text-xs text-white font-medium">
            ...
            <div className="mt-4 pt-4 border-t border-slate-100">
              ...
              <div className="flex -space-x-2">
                {pendingParticipants.slice(0, 6).map((p) => (
                  <div
                    key={p.id}
                    className="w-8 h-8 rounded-lg bg-slate-200 ring-2 ring-white flex items-center justify-center"
                  >
                ...
                {pendingParticipants.length > 6 && (
                  <div className="w-8 h-8 rounded-lg bg-slate-100 ring-2 ring-white flex items-center justify-center text-xs font-medium text-slate-600">
                    +{pendingParticipants.length - 6}
                  </div>
                )}
              </div>
        ...
        <div className="space-y-1">
          {competition.rule_total_count && (
            <div className="rule-item">
              <div className="rule-icon bg-emerald-500/10 text-emerald-600">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" />
                </svg>
              </div>
              <span className="text-sm font-medium text-slate-700">Total fish count</span>
            </div>
          )}
          {competition.rule_record_size && (
            <div className="rule-item">
              <div className="rule-icon bg-teal-500/10 text-teal-700">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4" />
                </svg>
              </div>
              <span className="text-sm font-medium text-slate-700">Biggest catch</span>
            </div>
          )}
          {competition.rule_top_x_biggest && (
            <div className="rule-item">
              <div className="rule-icon bg-amber-500/10 text-amber-600">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
                </svg>
              </div>
              <span className="text-sm font-medium text-slate-700">Top {competition.rule_top_x_biggest} combined</span>
            </div>
          )}
          {!competition.rule_total_count && !competition.rule_record_size && !competition.rule_top_x_biggest && (
            <p className="text-sm text-slate-500 py-2">No rules defined</p>
          )}
        </div>
      </div>
